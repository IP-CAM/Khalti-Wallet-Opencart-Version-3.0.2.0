...
class ControllerExtensionPaymentKhalti extends Controller
{
    ...
    public function index()
    {
        ...
        $getTransaction = json_decode($getTransaction, true);
		if(array_key_exists('records', $getTransaction)){
			foreach ($getTransaction['records'] as $t) {
				array_push($transaction, array(
					'idx'      => array_key_exists("idx", $t) ? $t['idx'] : null,
					'source'   => array_key_exists("user", $t) ? $t['user']['name'] : null,
					'amount'   => array_key_exists("amount", $t) ? $t['amount'] / 100 : null,
					'fee'      => array_key_exists("fee_amount", $t) ? $t['fee_amount'] / 100 : null,
					'date'     => date("Y/m/d H:m:s", strtotime($t['created_on'])),
					'type'     => array_key_exists("type", $t) ? $t['type']['name'] : null,
					'state'    => $t['refunded'] == true ? "Refunded" : $t['state']['name'],
					'refunded' => array_key_exists("refunded", $t) ? $t['refunded'] : null,
